...
while userConfirm != "no":
    ...
    if (R.status_code != 200): #checking for an OK response from API
        ...
        while data_valid == False:
            ...
            try:
                Useranswer = int(Useranswer)#trying to convert input to integer
                if Useranswer > len(choices) or Useranswer <= 0:
                    print("Please enter a valid option.")#if it fails the while loops starts again
                else:
                    data_valid = True
            except:
                print("Invalid input, alphabets are not allowed.")#if it fails the while loop start again
